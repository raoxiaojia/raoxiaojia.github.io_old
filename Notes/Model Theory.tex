...
\begin{defi} (3.4)\\
    ...
    \begin{equation*}
        ...
        \end{aligned}
    \end{equation*}
    (ii) If $M \subseteq N$, and $id:\mathcal{N} \to \mathcal{N}$ is an embedding, then $\mathcal{M}$ is a \emph{substructure} of $\mathcal{N}$.\\
...
\begin{rem} (3.9)\\
    ...
    $\bullet$ the cardinality of the set of $L$-formulas is $|L|+\omega$ (what is $|L|$ here -- size of symbol/fuction/constant sets?), where we abuse another notation that we use $\omega$ as cardinals (rather than ordinals) (note that the formulas are just strings of finite length);\\
    $\bullet$ if $A$ is a set of parameters in some structure, the cardinality of the set $L(A)$ is $|A|+|L|+\omega$.
\end{rem}
