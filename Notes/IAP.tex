...
\[
A^4 = \bigcup_{\varepsilon_i = \pm 1, 0 \leq m \leq 4} A^{\varepsilon_1}...A_{\varepsilon_m}
\]
...
\begin{proof}
It's trivial if $C=\phi$, so we may assume $\exists x \in C$. Defining $C' = C \{x\}$, we may also assume by inducion that $|C'UB| \leq R|C'U|$.\\
