...
Note that although this is quite a length proof, there's only one way we can go, i.e. verify everything we need.
...
\begin{lemma} (5.11)\\
    Suppose we are given an adjunction $\mathcal{C} \stackrel[G]{F}{\rightleftarrows} \mathcal{D}$, inducing a monad $\T$ on $\mathcal{C}$, then $K:\mathcal{D} \to \mathcal{C}^\T$ has a left adjoint provided, for every $\T$-algebra $(A,\alpha)$, the pair $FGFA \stackrel[\varepsilon_{FA}]{F\alpha}{\rightrightarrows} FA$ has a coqeualizer in $\mathcal{D}$.
    \begin{proof}
...
\subsection{Question 1}
This is intended to be a easy question, although you could spend a lot of time if you want to go into all the details.
...
Now suppose we are given $D:\mathcal{J} \to \mathcal{C}$ where $\mathcal{J}$ is finite and has aweakly initial object $j_0$. Let
...
where $E_1 \to D(j_0)$ is the equalizer of $D(\alpha_1)$ and $D(\beta_1)$, $E_2 \to E_1$ is tha equalizer of $E_1 \to D(j_0) \stackrel[D(\beta_2)]{D(\alpha_2)}{\rightrightarrows} D(j_2)$, and so on.\\
