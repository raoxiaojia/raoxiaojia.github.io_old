...
\begin{defi} (1.1)\\
    ...
    $\bullet$ (c) two operations domain, codomain assigining to each $f \in \mor\mathcal{C}$, a pair of objects, it's \emph{domain} and \emph{codomain}; we write $A \xrightarrow{f} B$ to mean \emph{$f$ is a morphism and $\dom f = A, \cod f = B$};\\
