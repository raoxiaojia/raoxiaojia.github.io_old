...
\begin{defi} (5.5)\\
    Given an algebra monad $\mathbb{T}$ on $\mathcal{C}$, the \emph{Kleisli category} $\mathcal{C}_\mathbb{T}$ has $\ob\mathcal{C}_\mathbb{T} = \ob\mathcal{C}$ (and because of this, we'll use green for morphisms in $\mathcal{D}_\mathbb{T}$. It might be useful to bring pens of different colours in the next few lectures), and morphisms
    \begin{tikzcd}
        A \arrow[r,green] & B
    \end{tikzcd}
    ...
    \begin{tikzcd}
        A \arrow[r,"f"] & TB \arrow[r,"Tg"] & TTC \arrow[r,"TTh"] \arrow[d,"\mu_C"] & TTTD \arrow[r,"T_{\mu D}"] \arrow[d,"\mu_{TD}"] & TTD \arrow[d,"\mu_D"]\\
