...
\section{Definitions an examples}
\begin{defi} (1.1)\\
    ...
    $\bullet$ (c) two operations domain, codomain assigining to each $f \in \mor\mathcal{C}$, a pair of objects, it's \emph{domain} and \emph{codomain}m we write $A \xrightarrow{f} B$ to mean \emph{$f$ is a morphism and $\dom f = A, \cod f = B$};\\
...
\begin{eg} (1.3)\\
    ...
    (c) The cateogry $Top$ has all topological spaces as objects, and continuous functions as morphisms.\\
    ...
    (i) The category $Rel$ has the same objects a \emph{set} (???), but morphisms $A \to B$ are arbitrary relations $R \subseteq A \times B$. Given $R$ and $S \subseteq B\times C$, we define $S \cdot R = \{(a,c) \in A \times C | (\exists b \in B) ((a,b) \in R, (b,c) \in S)\}$.\\
...
\begin{eg} (1.5)\\
    ...
    Given any group $G$ and any function $A \xrightarrow{f} UG$ (?), there's a unique homomorphism $FA \xrightarrow{F} G$ extending $f$.\\
    $F$ is a functor $Set\to Gp$: given $A \xrightarrow{f} B$, we define $Ff$ to be the unique homomorphism extending $A \xrightarrow{f} B \leftrightarrow UFB$ Functionality follows from uniqueness given $B \xrightarrow{f} C$. $F(gf)$ and $(Fg)(Ff)$ are both momoms extending $A \xrightarrow{f} B \xrightarrow{g} C \rightarrow UFC$.\\
